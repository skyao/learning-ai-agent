...
- **模型线在 2026 换了问法。** 上半年的关键词是**长程**：compaction 与 agent teams 把「跑得久、开多条线」收进核；三档划分与中档降价把「能自主跑」下放到更低的价位。年中之后关键词变成**验证**——[Opus 5](../opus-5/)、[GPT-6 Astra](../gpt-6-astra/) 都强调「确认做对了再收工」。核不再只是选下一步，也开始承担「这一步对不对」。
- **决定核心够用到可以暴露系统问题。** 当循环能跑数小时、能开 PR、能查数仓，瓶颈变成：凭证能不能进沙箱、出站要不要白名单、空闲后工作区是否还在、频道里谁有权转向。这些不是更多 CoT 能解决的。
...
- **模型的可用性掌握在自己手里。** 6 月出口管制后，依赖单一前沿模型的产品必须假设它会突然消失。可替换的核、沙箱、协议都不是可选项。
