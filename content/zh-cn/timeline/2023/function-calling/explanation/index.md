...
date: 2026-09-15
description: >
  工具调用从 prompt 把戏收成可校验 schema。协议，不是系统；循环仍由宿主写。
---
...
按 AI Agent 定义，**Function Calling 本身不是 Agent**。它是协议：模型↔工具的第一种稳定 schema。它没有规定多步循环，也不对环境行动——副作用出在你的服务器上。缺了它，循环仍然能用文本协议硬撑；有了它，循环才变得可产品化。不可信工具输出仍可注入非预期动作：公告已经写明，不是后来才发现的漏洞。协议不是系统；schema 到位，循环仍在宿主。
