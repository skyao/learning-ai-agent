...
各家用词不同，落到工程上几乎是同一句。拿来对照的，是 Simon Willison 写成的这一句：
...
本笔记的四要素就是把这句话拆开：谁在决策、是不是多步骤、有没有碰到环境、看没看反馈。Simon 也强调了停止条件（完成、失败、交给人）——这不是无限循环。

## 主要参考对象

经典教材（Russell & Norvig）仍然是另一套：任何能感知环境、按性能度量行动的东西。那套定义太过宽泛，已经写进 [什么不是AI Agent](/definition/what-is-not-ai-agent/)，这里不再展开。下面只列 LLM Agent 这一波里，比较有参考意义的几份：
...
| [ReAct, Yao et al.](https://arxiv.org/abs/2210.03629) | Thought ↔ Action ↔ Observation 交错。 | 这是循环的名字来源，不是产品定义。[什么是 AI Agent](/definition/what-is-ai-agent/) 里的最小循环就是在讲这篇。 |

## 和本笔记对齐的地方

工程侧这几年基本都在对齐，只是用词不同。对照下来，我认可这四条：

1. **决策核心是语言模型，不是写死的 if/else** — Anthropic / LangChain 说的 control flow。
2. **有环境行动（tools）**，不是只吐文本。
3. **有循环**：看结果，再决定下一步 — ReAct / Anthropic 的 loop。
4. **有停止条件**：目标完成、失败、或交给人。

OpenAI 也把 chatbot、单轮调用划出去，和我在「什么不是AI Agent」里的处理一致。Function Calling 我标记为「地基不是系统」：工具接口是模型能力，循环才是 Agent。社区里普遍这样区分。

弱自主也算 AI Agent。Anthropic、LangChain、OpenAI 都承认 human-in-the-loop。Agent 不是「完全没人管」。这一点我写进定义，不是为了放宽，而是不认可「必须全自动才叫 Agent」那种说法。
...
**「独立完成任务」容易被营销带跑。** OpenAI 的 *independently*、产品名里的 Agent，和工程上的 *tools in a loop* 经常不是一回事。所以我把定义拆成「什么是 AI Agent / 什么不是 AI Agent」两页，就是不认可「比较能干活的 AI」这种起名。
